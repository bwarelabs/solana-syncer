...
public class App {
  ...
  public static void main(String[] args) {
    ...
    if (readSource == null) {
      logger.severe(
          "Error: 'read-source' argument is required. Valid values for 'read-source' are 'bigtable' and 'local-files'.");
      return;
    }
    ...
    if (readSource.equals("cos")) {
      try {
        CosToHbaseWriter cosToHbaseWriter = new CosToHbaseWriter(properties, blocksStartKey, blocksLastKey);
        cosToHbaseWriter.write();

        CosUtils.shutdown();
      } catch (Exception e) {
        logger.severe(String.format("An error occurred while migrating data from COS: %s", e));
        e.printStackTrace();
      }
      logger.info("Done!");
      return;
    ...
    logger.severe("Error: Invalid 'read-source' argument. Valid values are ['bigtable', 'local-files', 'cos'].");
  }
}
