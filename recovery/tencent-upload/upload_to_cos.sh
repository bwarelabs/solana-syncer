...
if [ "$#" -ne 2 ]; then
    echo "Usage: $0 <SOURCE_DIRECTORY> <BUCKET_ALIAS>"
    exit 1
fi
...
find "$SOURCE_DIRECTORY" -type f -name "*.txt" | while read -r file; do
    ...
    # Set the full destination path in COS, including the bucket alias and hardcoded 'live_sync' path
    DEST_PATH="cos://$BUCKET_ALIAS/test_sync/$RELATIVE_PATH"
